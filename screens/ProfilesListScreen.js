...
export default function ProfilesListScreen({ navigation }) {
  ...
  // Aşağı çekince çalışacak fonksiyon [cite: 1019-1027]
  const onRefresh = async () => {
    setRefreshing(true);
    setProfiles([]); // Listeyi temizle
    setPage(1);      // Sayfayı başa al
    setHasMore(true);
    
    // API isteğini manuel yapıyoruz ki loading state karışmasın
    try {
        ...
        setRefreshing(false); // Yenileme ikonunu kapat
    }
  };
  ...
  // Liste boşsa gösterilecek bileşen [cite: 1000-1008]
  const renderEmpty = () => {
    if (loading) return null;
    return (
  ...
  // İlk yükleme ekranı [cite: 1038-1045]
  ...
  // Hata ekranı (sadece liste boşsa göster, doluysa altta hata kalsın)
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        // Yenileme kontrolü [cite: 1031-1033]
        refreshControl={
